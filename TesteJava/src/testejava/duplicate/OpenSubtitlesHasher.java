...
public class OpenSubtitlesHasher {
	...
	public static String computeHash(File file) throws IOException {
		...
		try {
			long head = computeHashForChunk(fileChannel.map(MapMode.READ_ONLY, 0,
					chunkSizeForFile));
	...
	public static String computeHash(Path file) {
		...
		try {
			...
			r = String.format("%016x", size + head + tail);
	...
	public static String computeHash(InputStream stream, long length) throws IOException {
		...
		long tail = computeHashForChunk(ByteBuffer.wrap(chunkBytes, chunkBytes.length
				- chunkSizeForFile, chunkSizeForFile));
